...
LIBS=`find lib/ \( -name '*.a' -o -name '*.so' -o -name '*.so.*' \) -not -name 'cyg*.dll'`
LIBS_CYG=`find lib/ -name 'cyg*.dll'`

echo '[YI] Install libraries'
echo rm -f "$LIBDIR"/libyices*
rm -f "$LIBDIR"/libyices*
install -t "$LIBDIR" $LIBS || failwith "cannot install libraries"
if [ -f lib/libyices.so* ] && [ ! -f lib/libyices.so ]; then
  echo '[YI] Symbolic link the last version of libyices'
  cd lib
  LAST_VERSION=`ls libyices.so* | tail -1`
  cd ..
  ln -s "$LIBDIR/$LAST_VERSION" "$LIBDIR/libyices.so"
fi
ln -s "$LIBDIR/libyices.so"
if [ -n "$LIBS_CYG" ]; then
...
rm -f "$INSTALL/include/yices*"
install -t "$INSTALL/include" -m 'a=r,u+w' include/*.h || failwith "cannot install headers"

echo '[YI] Install executable'
install -t "$BINDIR" bin/yices bin/yices-sat bin/yices-smt || failwith "cannot install executables"
