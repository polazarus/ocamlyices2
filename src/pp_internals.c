...
static ssize_t ocamlyices_pp_write (void *cookie_, const char *buf,
                                    size_t size) {
  ocamlyices_pp_cookie_t *cookie = (ocamlyices_pp_cookie_t *)cookie_;
  caml_callback(cookie->cb, caml_copy_stringn(size, buf));
  return size;
}

int ocamlyices_pp_with_callback(value v_cb, int (*pp_fun)(FILE *, void *),
                                void *arg) {
...
int ocamlyices_internal_pp_with_callback(value v_cb, int (*pp_fun)(FILE *,
    void *), void *arg) {
  ...
  if (ptc_res != 0) {
    close(pipefd[0]);
    close(pipefd[1]);
    _oy__error(); // cannot create thread
  }
